        ...
        for i, (img_id, v_img, r_img, verb, labels) in enumerate(train_loader):
            ...
            if total_steps % eval_frequency == 0:
                ...
                if max_score == dev_score_list[-1]:
                    torch.save(model.state_dict(), model_dir + "/{}_roleverb_special_{}.model".format( model_name, model_saving_name))
...
def main():
    ...
    parser.add_argument('--model', type=str, default='baseline0grid_imsitu_roleverb_special')
    ...
    parser.add_argument('--place_module', type=str, default='', help='pretrained place module')
    ...
    constructor = 'build_%s' % 'baseline0grid_imsitu_singlerole'
    agent_model = getattr(base_model, constructor)(train_set, args.num_hid, len(encoder.agent_label_list), encoder)
    place_model = getattr(base_model, constructor)(train_set, args.num_hid, len(encoder.place_label_list), encoder)
    ...
    model = getattr(base_model, constructor)(train_set, args.num_hid, encoder.get_num_verbs(), encoder, agent_model, place_model, args.num_iter_verb)
    ...
    if args.evaluate:
        ...
        with open('createdq_roleverbspecial.json', 'w') as fp:

...
from imsitu_loader import imsitu_loader_rolename_buatt
...
def train(model, train_loader, dev_loader, traindev_loader, optimizer, scheduler, max_epoch, model_dir, encoder, gpu_mode, clip_norm, lr_max, model_name, model_saving_name, args,eval_frequency=4000):
    ...
    for epoch in range(max_epoch):
        ...
        for i, (_, img, verb, questions, labels) in enumerate(train_loader):
            ...
            role_predict = pmodel(img, questions, labels)
            ...
            if total_steps % eval_frequency == 0:
                ...
                if max_score == dev_score_list[-1]:
                    torch.save(model.state_dict(), model_dir + "/{}_roles_independent_rolenames{}.model".format( model_name, model_saving_name))
...
def eval(model, dev_loader, encoder, gpu_mode, write_to_file = False):
    ...
    with torch.no_grad():
        ...
        for i, (img_id, img, verb, questions, labels) in enumerate(dev_loader):
            ...
            role_predict = model(img, questions, None)
...
def main():
    ...
    dict_path = 'data/dictionary_imsitu_rolenames.pkl'
    dictionary = Dictionary.load_from_file(dict_path)
    w_emb_path = 'data/glove6b_init_imsitu_rolenames300d.npy'

    train_set = imsitu_loader_rolename_buatt(imgset_folder, train_set, encoder, dictionary, 'train', encoder.train_transform)
    ...
    dev_set = imsitu_loader_rolename_buatt(imgset_folder, dev_set, encoder, dictionary, 'val', encoder.dev_transform)
    ...
    test_set = imsitu_loader_rolename_buatt(imgset_folder, test_set, encoder, dictionary, 'test', encoder.dev_transform)

...
from imsitu_loader import imsitu_loader_verb_buatt_common
...
def train(model, train_loader, dev_loader, traindev_loader, optimizer, scheduler, max_epoch, model_dir, encoder, gpu_mode, clip_norm, lr_max, model_name, model_saving_name, args,eval_frequency=4000):
    ...
    for epoch in range(max_epoch):
        ...
        for i, (_, img, verb, questions) in enumerate(train_loader):
            ...
            verb_predict = pmodel(img, questions)
            ...
            if total_steps % eval_frequency == 0:
                ...
                if max_score == dev_score_list[-1]:
                    torch.save(model.state_dict(), model_dir + "/{}_verb_commonq_{}.model".format( model_name, model_saving_name))
...
def eval(model, dev_loader, encoder, gpu_mode, write_to_file = False):
    ...
    with torch.no_grad():
        mx = len(dev_loader)
        for i, (img_id, img, verb, questions) in enumerate(dev_loader):
            ...
            verb_predict = model(img, questions)
...
def main():
    ...
    parser.add_argument('--model', type=str, default='baseline0grid_imsitu_verb')
    parser.add_argument('--output', type=str, default='saved_models/exp0')
    parser.add_argument('--batch_size', type=int, default=64)
    parser.add_argument('--seed', type=int, default=1111, help='random seed')

    #verb special args
    parser.add_argument('--commonq_type', type=str)
    parser.add_argument('--train_word_embd', action='store_true')
    parser.add_argument('--train_q_embd', action='store_true')
    ...
    dict_path = 'data/dictionary_imsitu_verbcommon_' + args.commonq_type + '.pkl'
    dictionary = Dictionary.load_from_file(dict_path)
    w_emb_path = 'data/glove6b_init_imsitu_verbcommon_' + args.commonq_type +'_300d.npy'

    train_set = imsitu_loader_verb_buatt_common(imgset_folder, train_set, encoder, dictionary, 'train', args.commonq_type, encoder.train_transform)

    constructor = 'build_%s' % args.model
    model = getattr(base_model, constructor)(train_set, args.num_hid, encoder.get_num_verbs(), encoder)
    ...
    dev_set = imsitu_loader_verb_buatt_common(imgset_folder, dev_set, encoder, dictionary, 'val', args.commonq_type, encoder.dev_transform)
    ...
    test_set = imsitu_loader_verb_buatt_common(imgset_folder, test_set, encoder, dictionary, 'test', args.commonq_type,encoder.dev_transform)

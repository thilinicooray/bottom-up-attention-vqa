import torch
from imsitu_encoder_roleq_agent import imsitu_encoder
from imsitu_loader import imsitu_loader_roleq_buatt_agent
...
def train(model, train_loader, dev_loader, traindev_loader, optimizer, scheduler, max_epoch, model_dir, encoder, gpu_mode, clip_norm, lr_max, model_name, model_saving_name, args,eval_frequency=4):
    ...
    for epoch in range(max_epoch):
        ...
        for i, (_, img, verb, questions, labels) in enumerate(train_loader):
            ...
            agent_predict, loss1 = pmodel(img, questions, labels, verb)
            ...
            if total_steps % eval_frequency == 0:
                ...
                if max_score == dev_score_list[-1]:
                    torch.save(model.state_dict(), model_dir + "/{}_agent_ftall{}.model".format( model_name, model_saving_name))
            ...
            del agent_predict, loss, img, verb, labels
            #break
...
def eval(model, dev_loader, encoder, gpu_mode, write_to_file = False):
    ...
    with torch.no_grad():
        ...
        for i, (img_id, img, verb, questions, labels) in enumerate(dev_loader):
            ...
            agent_predict, _ = model(img, questions, labels, verb)
            '''loss = model.calculate_eval_loss(verb_predict, verb, role_predict, labels)
            val_loss += loss.item()'''
            top1.add_point_agent_only(agent_predict, labels)
            top5.add_point_agent_only(agent_predict, labels)

            del agent_predict, img, verb, labels
            break
...
def main():
    ...
    parser.add_argument('--model', type=str, default='baseline0grid_imsitu_agent')
    ...
    train_set = imsitu_loader_roleq_buatt_agent(imgset_folder, train_set, encoder, dictionary, 'train', encoder.train_transform)

    constructor = 'build_%s' % args.model
    model = getattr(base_model, constructor)(train_set, args.num_hid, len(encoder.agent_label_list), encoder, args.num_iter)
    ...
    dev_set = imsitu_loader_roleq_buatt_agent(imgset_folder, dev_set, encoder, dictionary, 'val', encoder.dev_transform)
    ...
    test_set = imsitu_loader_roleq_buatt_agent(imgset_folder, test_set, encoder, dictionary, 'test', encoder.dev_transform)

...
from imsitu_loader import imsitu_loader_buatt_roleonly_indiloss
...
def train(model, train_loader, dev_loader, traindev_loader, optimizer, scheduler, max_epoch, model_dir, encoder, gpu_mode, clip_norm, lr_max, model_name, model_saving_name, args,eval_frequency=4000):
    ...
    for epoch in range(max_epoch):
        ...
        mx = len(train_loader)
        for i, (_, img, verb, questions, labels, label_scores) in enumerate(train_loader):
            ...
            role_predict, loss1 = pmodel(img, questions, label_scores, verb)
            ...
            if total_steps % eval_frequency == 0:
                ...
                if max_score == dev_score_list[-1]:
                    torch.save(model.state_dict(), model_dir + "/{}_roles_iter_ftall_indiloss{}.model".format( model_name, model_saving_name))
...
def eval(model, dev_loader, encoder, gpu_mode, write_to_file = False):
    ...
    with torch.no_grad():
        mx = len(dev_loader)
        for i, (img_id, img, verb, questions, labels, label_scores) in enumerate(dev_loader):
            ...
            role_predict, _ = model(img, questions, label_scores, verb)
...
def main():
    ...
    parser.add_argument('--model', type=str, default='baseline0grid_imsitu_roleiter_indiloss')
    ...
    train_set = imsitu_loader_buatt_roleonly_indiloss(imgset_folder, train_set, encoder, dictionary, 'train', encoder.train_transform)
    ...
    dev_set = imsitu_loader_buatt_roleonly_indiloss(imgset_folder, dev_set, encoder, dictionary, 'val', encoder.dev_transform)
    ...
    test_set = imsitu_loader_buatt_roleonly_indiloss(imgset_folder, test_set, encoder, dictionary, 'test', encoder.dev_transform)

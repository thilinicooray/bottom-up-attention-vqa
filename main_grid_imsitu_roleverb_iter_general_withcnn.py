...
def train(model, train_loader, dev_loader, traindev_loader, optimizer, scheduler, max_epoch, model_dir, encoder, gpu_mode, clip_norm, lr_max, model_name, model_saving_name, args,eval_frequency=4):
    ...
    for epoch in range(max_epoch):
        ...
        for i, (img_id, img, verb, questions, labels) in enumerate(train_loader):
            ...
            verb_predict, loss1 = pmodel(img_id, img, verb, labels)
            loss = loss1
            ...
            if total_steps % eval_frequency == 0:
                ...
                if max_score == dev_score_list[-1]:
                    torch.save(model.state_dict(), model_dir + "/{}_roleverb_iter_general_withcnn_{}.model".format( model_name, model_saving_name))
...
def main():
    ...
    parser.add_argument('--model', type=str, default='baseline0grid_imsitu_roleverb_general_with_cnn')
    ...
    if args.use_pretrained_buatt:
        ...
        utils_imsitu.load_net_ban(args.pretrained_buatt_model, [model], ['module'], ['w_emb', 'role_module', 'classifier'])

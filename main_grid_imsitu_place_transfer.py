...
from imsitu_loader import imsitu_loader_roleq_buatt_place
        ...
        for i, (_, img, verb, questions, labels) in enumerate(train_loader):
            ...
            if total_steps % eval_frequency == 0:
                ...
                if max_score == dev_score_list[-1]:
                    torch.save(model.state_dict(), model_dir + "/{}_place_ftall{}.model".format( model_name, model_saving_name))
...
def eval(model, dev_loader, encoder, gpu_mode, write_to_file = False):
    ...
    with torch.no_grad():
        ...
        for i, (img_id, img, verb, questions, labels) in enumerate(dev_loader):
            ...
            place_predict, _ = model(img, questions, labels, verb)
...
def main():
    ...
    train_set = imsitu_loader_roleq_buatt_place(imgset_folder, train_set, encoder, dictionary, 'train', encoder.train_transform)

    constructor = 'build_%s' % args.model
    model = getattr(base_model, constructor)(train_set, args.num_hid, len(encoder.place_label_list), encoder)
    ...
    dev_set = imsitu_loader_roleq_buatt_place(imgset_folder, dev_set, encoder, dictionary, 'val', encoder.dev_transform)
    ...
    test_set = imsitu_loader_roleq_buatt_place(imgset_folder, test_set, encoder, dictionary, 'test', encoder.dev_transform)

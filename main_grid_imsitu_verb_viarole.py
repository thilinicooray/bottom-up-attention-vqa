...
def train(model, train_loader, dev_loader, traindev_loader, optimizer, scheduler, max_epoch, model_dir, encoder, gpu_mode, clip_norm, lr_max, model_name, model_saving_name, args,eval_frequency=4000):
    ...
    for epoch in range(max_epoch):
        ...
        mx = len(train_loader)
        for i, (img_id, img, verb, labels, label_scores) in enumerate(train_loader):
            ...
            verb_predict, loss1 = pmodel(img, labels, verb)
...
def eval(model, dev_loader, encoder, gpu_mode, write_to_file = False):
    ...
    with torch.no_grad():
        ...
        for i, (img_id, img, verb, labels, label_scores) in enumerate(dev_loader):
            ...
            verb_predict, _ = model(img, labels, verb)
...
def main():
    ...
    parser.add_argument('--model', type=str, default='baseline0grid_imsitu_roleiter_with_cnn_newmodel')

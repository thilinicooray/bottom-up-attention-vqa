...
def eval(model, dev_loader, encoder, gpu_mode, write_to_file = False):
    ...
    with torch.no_grad():
        ...
        for i, (img_id, v_img, r_img, verb, labels) in enumerate(dev_loader):
            ...
            verb_predict,loss  = model.forward_eval(img_id, v_img, r_img, verb, labels)
...
def main():
    ...
    parser.add_argument('--model', type=str, default='baseline0grid_imsitu_roleverb_general_gtq_train_beam')
    ...
    model = getattr(base_model, constructor)(train_set, args.num_hid, encoder.get_num_verbs(), encoder, role_model,
                                             args.num_iter_verb, args.beam_size)
